...
const CONTACT_EMAIL = "hello@5digea.com";
...
export default function ContactPage() {
  ...
  // The API has no "general enquiry" message type yet, so the form opens
  // the visitor's email app with everything pre-filled (see README).
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    const next: Partial<Record<Field, string>> = {};
    if (!form.name.trim()) next.name = t("contact.form.errorName");
    if (!EMAIL_RE.test(form.email.trim()))
      next.email = t("contact.form.errorEmail");
    if (form.message.trim().length < 10)
      next.message = t("contact.form.errorMessage");
    setErrors(next);

    if (Object.keys(next).length) {
      toast(Object.values(next)[0] as string, "error");
      return;
    }

    const subject = form.subject.trim() || t("contact.form.title");
            ...
            <div className="space-y-4">
              <div className="rounded-md border border-[#b99a62]/20 bg-white/40 p-7 backdrop-blur-sm">
                ...
                <a
                  ...
                  className="mt-2 block font-serif text-xl font-light text-[#30251f] transition-colors hover:text-[#a47e43]"
                  dir="ltr"
                >
                  hello@5digea.com
                </a>
